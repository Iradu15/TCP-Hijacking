...
iptables -t nat -A POSTROUTING -j MASQUERADE  # SEVERUL face handshake cu routerul deoracere ip-ul cientului e mascat ca fiind cel al routerului 
# replaces the source IP address and port of outgoing packets with the IP address of the outgoing interface.
echo "router script succes"